...
def create_brand_analysis_guide_pdf():
    """
    Create a comprehensive brand analysis guide PDF for users.
    """
    try:
        from reportlab.lib.pagesizes import A4
        ...
        pdf_path = os.path.join(static_dir, "brand-analysis-guide.pdf")
        ...
        # Custom styles
        title_style = ParagraphStyle('CustomTitle', parent=styles['Title'], fontSize=20, spaceAfter=30, textColor=colors.HexColor('#2095EA'))
        heading_style = ParagraphStyle('CustomHeading', parent=styles['Heading1'], fontSize=16, spaceAfter=15, textColor=colors.HexColor('#1f2937'))
        subheading_style = ParagraphStyle('CustomSubHeading', parent=styles['Heading2'], fontSize=14, spaceAfter=10, textColor=colors.HexColor('#4f46e5'))
        body_style = ParagraphStyle('CustomBody', parent=styles['Normal'], fontSize=11, spaceAfter=10, alignment=TA_JUSTIFY)
        footer_style = ParagraphStyle('Footer', parent=styles['Normal'], fontSize=10, textColor=colors.HexColor('#6b7280'), alignment=TA_CENTER)
        
        # Header
        story.append(Paragraph("AI Brand Visibility Analysis Guide", title_style))
        story.append(Paragraph("Understanding Your Brand's Presence in AI Search Results", subheading_style))
        story.append(Spacer(1, 20))
        
        # Introduction
        story.append(Paragraph("Introduction", heading_style))
        intro_text = """
        This guide will help you understand how to use the AI Brand Visibility Checker effectively and interpret your results. 
        As AI-powered search engines become more prevalent, understanding your brand's visibility in these platforms is crucial 
        for modern digital marketing success.
        """
        story.append(Paragraph(intro_text, body_style))
        story.append(Spacer(1, 15))
        
        # How to Use the Tool
        story.append(Paragraph("How to Use the AI Brand Visibility Checker", heading_style))
        
        story.append(Paragraph("1. Input Your Details", subheading_style))
        step1_text = """
        • <b>Search Keyword:</b> Enter the industry or service you want to analyse (e.g., "web development", "digital marketing", "accounting services")
        <br/>• <b>Brand Name:</b> Enter your company or brand name exactly as you want it searched
        <br/>• <b>Search Query Options:</b> Choose between auto-generated queries or create 1-5 custom search queries
        """
        story.append(Paragraph(step1_text, body_style))
        
        story.append(Paragraph("2. Choose Your Analysis Mode", subheading_style))
        step2_text = """
        • <b>Standard Mode:</b> Comprehensive analysis with full competitor detection
        <br/>• <b>Performance Mode:</b> Faster analysis with slightly less detailed competitor information
        <br/>• <b>Demo Mode:</b> Quick test with sample data for demonstration purposes
        """
        ...
        story.append(Paragraph("Performance Grade", subheading_style))
        grade_text = """
        Your brand receives a letter grade (A+, A, B+, B, C+, C, D, F) based on how frequently it appears in AI search results. 
        This grade reflects your overall visibility and prominence when people search for services in your industry.
        """
        story.append(Paragraph(grade_text, body_style))
        
        story.append(Paragraph("Visibility Percentage", subheading_style))
        visibility_text = """
        This shows the raw percentage of search queries where your brand was mentioned. A higher percentage indicates 
        better visibility across different AI platforms and search contexts.
        """
        story.append(Paragraph(visibility_text, body_style))
        
        story.append(Paragraph("Sentiment Analysis", subheading_style))
        sentiment_text = """
        • <b>Overall Sentiment Summary:</b> A narrative overview of how your brand is perceived
        <br/>• <b>Key Strengths:</b> Specific positive attributes mentioned about your brand
        <br/>• <b>Areas for Improvement:</b> Potential weaknesses or areas to focus on
        <br/>• <b>Sentiment Drivers:</b> Key topics that influence your brand perception with strength ratings
        """
        story.append(Paragraph(sentiment_text, body_style))
        
        story.append(Paragraph("Competitor Analysis", subheading_style))
        competitor_text = """
        The competitor insights show how your brand compares to others in your industry. The pie chart displays 
        market share based on mention frequency, while the analysis provides strategic insights about your competitive position.
        """
        story.append(Paragraph(competitor_text, body_style))
        
        story.append(Spacer(1, 15))
        
        # Best Practices
        story.append(Paragraph("Best Practices for Improving Visibility", heading_style))
        
        practices_data = [
            ["Strategy", "Description", "Impact"],
            ["Structured Content", "Use clear headings, fact-based information, and direct answers to common questions", "High"],
            ["Question-Based Keywords", "Target natural language queries people actually ask", "High"],
            ["Schema Markup", "Implement structured data to help AI understand your content", "Medium"],
            ["Brand Attribution", "Explicitly mention your brand name with key services and expertise", "High"],
            ["Fresh Content", "Keep content updated with current information and dates", "Medium"],
            ["Authority Building", "Gain citations and backlinks from reputable industry sources", "High"],
        ]
        
        practices_table = Table(practices_data, colWidths=[2*inch, 3*inch, 1*inch])
        practices_table.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#2095EA')),
        ...
        story.append(Paragraph("Implementing Recommendations", heading_style))
        implementation_text = """
        The recommendations section provides specific, actionable strategies tailored to your industry. These are based on 
        current best practices for Generative Engine Optimisation (GEO) and Answer Engine Optimisation (AEO). 
        Prioritise high-impact changes first, then work through medium and low priority items systematically.
        """
        ...
        # Footer information
        story.append(Paragraph("About This Tool", heading_style))
        about_text = """
        The AI Brand Visibility Checker is developed by A.P. Web Solutions to help Australian businesses understand 
        and improve their presence in AI-powered search results. For custom digital marketing strategies and 
        professional web development services, visit www.apwebsolutions.com.au
        """
        ...
        # Footer
        story.append(Paragraph("© 2025 A.P. Web Solutions | www.apwebsolutions.com.au", footer_style))
        
        # Build PDF
        doc.build(story)
        print(f"[INFO] Brand analysis guide PDF created at: {pdf_path}")
        
    except Exception as e:
        print(f"[Error] Failed to create brand analysis guide PDF: {e}")

# Create the guide PDF on startup
create_brand_analysis_guide_pdf()
...
def generate_pdf_report(brand_name, keyword, main_score, summary, avg_sent, perception, summary_df, comp_insights, recommendations, pie_img_path):
    ...
    story = []
    warning_message = ""
    # Add company logo and website link at the very top
    ...
    if os.path.exists(logo_path):
        try:
            ...
            warning_message = "Warning: Could not load logo image."
            print(f"[Warning] {warning_message}: {e}")
    else:
        warning_message = "Warning: Logo file not found, so it was not included in the PDF report."
        print(f"[Warning] Logo file not found at {logo_path}, skipping.")
    
    # Add website link without hyperlink to avoid ReportLab URL issues
    story.append(Paragraph(
        "www.apwebsolutions.com.au",
        ParagraphStyle('LogoLink', parent=styles['Normal'], alignment=TA_CENTER, fontSize=12, textColor=colors.blue)
    ))
    story.append(Spacer(1, 18))
    # Title
    story.append(Paragraph(aus_spell("AI Brand Visibility Report"), styles['Title']))
    story.append(Spacer(1, 8))
    # Brand and Keyword right below the title
    story.append(Paragraph(aus_spell(f"<b>Brand:</b> {brand_name}"), styles['Normal']))
    story.append(Paragraph(aus_spell(f"<b>Keyword:</b> {keyword}"), styles['Normal']))
    story.append(Spacer(1, 18))
    # Main Score Section
    story.append(Paragraph(aus_spell("Your Visibility %"), styles['Heading2']))
    story.append(Spacer(1, 8))
    story.append(Paragraph(main_score, ParagraphStyle('Score', parent=styles['Heading2'], alignment=TA_CENTER)))
    story.append(Spacer(1, 18))
    # Score Summary
    story.append(Paragraph(summary, styles['Normal']))
    story.append(Spacer(1, 18))
    # Sentiment
    story.append(Paragraph("Average Sentiment", styles['Heading2']))
    story.append(Spacer(1, 6))
    story.append(Paragraph(f"{avg_sent:.2f}", styles['Normal']))
    story.append(Spacer(1, 12))
    story.append(Paragraph("Sentiment Summary", styles['Heading2']))
    story.append(Spacer(1, 6))
    add_markdown_to_story(perception, story, styles['Normal'])
    story.append(Spacer(1, 18))
    # Brand Mentions Table
    if not summary_df.empty:
        story.append(Paragraph(aus_spell("Brand Mentions Table"), styles['Heading2']))
        story.append(Spacer(1, 6))
        # Build table data with check/cross symbols for brand mentions
        ...
        table = Table(table_data, repeatRows=1)  # No colWidths for auto-expansion
        table.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#4f46e5')),
            ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
            ('BOTTOMPADDING', (0, 0), (-1, 0), 12),
    ...
    # Competitor Insights
    if comp_insights:
        story.append(Paragraph(aus_spell("Competitor Insights"), styles['Heading2']))
        story.append(Spacer(1, 6))
        # Add competitor pie chart image if available
        if pie_img_path and os.path.exists(pie_img_path):
            try:
                pie_img = Image(pie_img_path, width=6.5*inch, height=6.5*inch)
            ...
            print(f"[Warning] Pie chart image not found at: {pie_img_path}")
        # Render markdown tables and markdown text
        if '|' in comp_insights and '\n---' in comp_insights:
            parts = re.split(r'(\n\|.+\n\|[\-\s\|]+\n(?:\|.*\n)+)', comp_insights, maxsplit=1)
            add_markdown_to_story(parts[0], story, styles['Normal'])
            if len(parts) > 1:
                add_markdown_table_to_story(parts[1], story)
        elif '|' in comp_insights:
            add_markdown_table_to_story(comp_insights, story)
        else:
            add_markdown_to_story(comp_insights, story, styles['Normal'])
        story.append(Spacer(1, 18))
    # Recommendations
    if recommendations:
        story.append(Paragraph(aus_spell("GEO/AEO Best Practices"), styles['Heading2']))
        story.append(Spacer(1, 6))
        add_markdown_to_story(recommendations, story, styles['Normal'])
        story.append(Spacer(1, 18))
    # Footer branding and CTA (without hyperlinks to avoid ReportLab issues)
    story.append(Spacer(1, 30))
    story.append(Paragraph(
        "Generated by A.P. Web Solutions<br/>"
        "Ready to improve your AI visibility? Contact us: www.apwebsolutions.com.au",
        ParagraphStyle('Footer', parent=styles['Normal'], fontSize=10, alignment=TA_CENTER)
    ))
    def add_footer(canvas, doc):
        footer_text = (
            "Generated by A.P. Web Solutions    "
            "Ready to improve your AI visibility? Contact us: www.apwebsolutions.com.au"
        )
        width, height = A4
        canvas.saveState()
        ...
        canvas.drawRightString(width - 40, 20, page_num)
        canvas.restoreState()
